...
MppFrameFormat ConvertToMppPixFmt(const PixelFormat &fmt) {
  ...
  static MppFrameFormat mpp_fmts[PIX_FMT_NB] = {
      ...
      [PIX_FMT_RGBA8888] = MPP_FMT_RGBA8888,
...
MppCodingType GetMPPCodingType(const std::string &data_type) {
  if (data_type == VIDEO_H264)
    ...
    return MPP_VIDEO_CodingMJPEG;
  RKMEDIA_LOGI("mpp decoder TODO for %s\n", data_type.c_str());
...
MPPContext::~MPPContext() {
  if (mpi) {
    ...
    mpp_destroy(ctx);
    ctx = NULL;
    RKMEDIA_LOGI("mpp destroy ctx done\n");
  }
  if (frame_group) {
    mpp_buffer_group_put(frame_group);
    frame_group = NULL;
    RKMEDIA_LOGI("mpp buffer group free done\n");
  }
}
...
MPP_RET init_mpp_buffer(MppBuffer &buffer,
  ...
  if (fd >= 0) {
    ...
    if (ret) {
      RKMEDIA_LOGI("import input picture buffer failed\n");
      goto fail;
    }
  } else {
    if (frame_size == 0)
    ...
    if (ret) {
      RKMEDIA_LOGI("allocate output stream buffer failed\n");
      goto fail;
    }
  }
...
MPP_RET init_mpp_buffer_with_content(MppBuffer &buffer,
  ...
  if (fd < 0 && ptr) {
    ...
    // RKMEDIA_LOGD("extra time-consuming memcpy to mpp, size=%d!\n", size);
...
MPP_RET mpi_enc_gen_ref_cfg(MppEncRefCfg ref, RK_S32 gop_mode, RK_S32 gop_len,
  ...
  default: {
    RKMEDIA_LOGE("MPP: unsupport gop mode %d\n", gop_mode);
  } break;
  }
